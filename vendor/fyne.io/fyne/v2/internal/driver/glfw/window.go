...
func (w *window) Show() {
	async.EnsureMain(func() {
		...
		// Plex Cover Manager patch:
		// GLFW/Windows can display the native window before the first GL frame
		// is swapped, causing a short white flash. Paint once while the window is
		// still hidden so the first visible frame already contains app content.
